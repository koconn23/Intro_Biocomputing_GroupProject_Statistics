...
size = 24 # Creates a random generation of 24 x-values between 0 and 50 
  outputx[,sim_number] #### A statistical power analysis ####
...
sigma <- c(1,2,4,6,8,12,16,24) ###### DO WE USE THIS FOR THIS FIRST PART OF THE CODE? I just wanted to make sure before I deleted it ######
...
outputy<-as.data.frame(matrix(0, ncol=10, nrow=24)) # Empty matrix to compile the 10 reiterations of the 24 randomly generated y variables
youtput <-list() # Empty file to compile the output matrices into a list

for (sim_number in 1:num_sims){ # This starts your for loop
  x = sample(x = 0:50, size = 24) 
  ...
  outputy[,sim_number] <- y # Puts the randomly generated y-values with 10 iterations into empty matrix created before the for-loop 
}
...
value.mat <- matrix(NA,8,10)
par.mat <- array(NA,dim = c(2,8,10))
...
for(sigma_number in 1:8){
  #for(n_per_sim in 1:24){
    for(num_sims in 1:10){

#Null model function

initialGuess = c(1,1)
null = optim(par=initialGuess,fn=nllnull,y=as.matrix(youtput[[sigma_number]][,num_sims]))
#nllresults[[num_sims]]  <- null
#print(nllresults)

value.mat[sigma_number,num_sims] <- null$value

par.mat[,sigma_number,num_sims] <- null$par

    }
  #}
}

lapply(nllresults,x$par)
