...
###### For loop to add noise to the y-values based on the sigma values given
  ...
  youtput[[sigma_number]] = outputy + rnorm(length(outputy), mean=0, sd=sigma[sigma_number])
  }
```
...
for(sigma_number in 1:8){
    for(num_sims in 1:10){
        initialGuess = rnorm(3,10,2)
        linear = optim(par=initialGuess,fn=nlllinear, x=as.matrix(outputx),
                     y=as.matrix(youtput[[sigma_number]][,num_sims]))
...
for(sigma_number in 1:8){ # Start of for loop 1
    for(num_sims in 1:10){ # Start of for loop 2
      ...
      q = 2*(linear.value.mat[sigma_number,num_sims]-null.value.mat[sigma_number,num_sims]) # Calculating the q-value for each cell - subtracting the linear value from the null value and multiplying by 2
...
for (num_sims in 1:10){
  print(mean(lrt.value[,num_sims]))
}
```
Because the average p-value across all sigma values is less than 0.05, we can reject the null model.
...
outputy2 <-as.data.frame(matrix(0, ncol=10, nrow=24))
youtput2 <-list()

for (sigma_number in 1:sigma_num){
  outputy2 = y2 + rnorm(length(y2), mean=0, sd=sigma[sigma_number])
    
  youtput2[[sigma_number]] <- outputy2
}
```

We then subset the data and ran each subset of data through the null and linear model.

```{r}

# Subsetting the data by x-value/trial
youtput2.1 = lapply(youtput2, function(x) x[1:12,,drop=FALSE])
youtput2.2 = lapply(youtput2, function(x) x[13:24,,drop=FALSE])

# Output matrices/arrays for null model values
A2.1.null.value.mat <- matrix(NA,8,10)
A2.1.null.par.mat <- array(NA,dim = c(2,8,10))
A2.2.null.value.mat <- matrix(NA,8,10)
A2.2.null.par.mat <- array(NA,dim = c(2,8,10))
...
### Null model for rows 1-12 (test value = 13)
for(sigma_number in 1:8){
    for(num_sims in 1:10){
        initialGuess = rnorm(2,-10)
        null = optim(par=initialGuess,fn=nllnull,
                     y=as.matrix(youtput2.1[[sigma_number]][,num_sims]))

        A2.1.null.value.mat[sigma_number,num_sims] <- null$value

        A2.1.null.par.mat[,sigma_number,num_sims] <- null$par
    }
}

### Null model For rows 13-24 (test value = 38)
for(sigma_number in 1:8){
    for(num_sims in 1:10){
        initialGuess = rnorm(2,-10)
        null = optim(par=initialGuess,fn=nllnull,
                     y=as.matrix(youtput2.2[[sigma_number]][,num_sims]))

        A2.2.null.value.mat[sigma_number,num_sims] <- null$value

        A2.2.null.par.mat[,sigma_number,num_sims] <- null$par
    }
}

# Output matrices/arrays for linear model values
A2.1.linear.value.mat <- matrix(NA,8,10)
A2.1.linear.par.mat <- array(NA,dim = c(3,8,10))
A2.2.linear.value.mat <- matrix(NA,8,10)
A2.2.linear.par.mat <- array(NA,dim = c(3,8,10))

### Linear model for rows 1-12 (test value = 13)
for(sigma_number in 1:8){
    for(num_sims in 1:10){
        initialGuess = rnorm(3,10,2)
        linear = optim(par=initialGuess,fn=nlllinear, x=as.matrix(x_values2),
                     y=as.matrix(youtput2.1[[sigma_number]][,num_sims]))

        A2.1.linear.value.mat[sigma_number,num_sims] <- linear$value

        A2.1.linear.par.mat[,sigma_number,num_sims] <- linear$par
    }
}

### Linear model for rows 13-24 (test value = 38)
for(sigma_number in 1:8){
    for(num_sims in 1:10){
        initialGuess = rnorm(3,10,2)
        linear = optim(par=initialGuess,fn=nlllinear, x=as.matrix(x_values2),
        ...
        A2.2.linear.value.mat[sigma_number,num_sims] <- linear$value

        A2.2.linear.par.mat[,sigma_number,num_sims] <- linear$par
    }
}
```

We then conducted a likelihood ratio test between the null and linear model for our first x-value (13).

```{r}
#### Likelihood Ratio Tests

#### Test value = 13 (null vs linear model)
lrt.value.13 <- matrix(NA,8,10) # Empty matrix to store the p-values

for(sigma_number in 1:8){ # Start of for loop 1
    for(num_sims in 1:10){ # Start of for loop 2
      
      q = 2*(A2.1.linear.value.mat[sigma_number,num_sims]-A2.1.null.value.mat[sigma_number,num_sims]) # Calculating the q-value for each cell - subtracting the linear value from the null value and multiplying by 2
      lrt = pchisq(q=q, df=1, lower.tail=FALSE) # Conducting the likelihood ratio test
      lrt.value.13[sigma_number,num_sims] <- lrt # Storing the outputs from the likelihood ratio test in the matrix that was created earlier
    }
  }

# Average p-value across monte carlos
for (num_sims in 1:10){
  print(mean(lrt.value.13[,num_sims]))
}
```
Because all p-values are smaller than 0.05 we can reject the null model.

We then conducted a likelihood ratio test between the null and linear model for our second x-value (38).

```{r}

#### Test value = 38
lrt.value.38 <- matrix(NA,8,10) # Empty matrix to store the p-values

for(sigma_number in 1:8){ # Start of for loop 1
    for(num_sims in 1:10){ # Start of for loop 2
      
      q = 2*(A2.2.linear.value.mat[sigma_number,num_sims]-A2.2.null.value.mat[sigma_number,num_sims]) # Calculating the q-value for each cell - subtracting the linear value from the null value and multiplying by 2
      lrt = pchisq(q=q, df=1, lower.tail=FALSE) # Conducting the likelihood ratio test
      lrt.value.38[sigma_number,num_sims] <- lrt # Storing the outputs from the likelihood ratio test in the matrix that was created earlier
    }
  }

# Average p-value across monte carlos
for (num_sims in 1:10){
  print(mean(lrt.value.38[,num_sims]))
}
```
Because all p-values are smaller than 0.05 we can reject the null model.
...
for(sigma_number in 1:8){ # Start of for loop 1
    for(num_sims in 1:10){ # Start of for loop 2
      ...
      q = -2*(A2.2.linear.value.mat[sigma_number,num_sims]-A2.1.linear.value.mat[sigma_number,num_sims]) # Calculating the q-value for each cell - subtracting the linear value from the null value and multiplying by 2
      lrt = pchisq(q=q, df=1, lower.tail=FALSE) # Conducting the likelihood ratio test
...
# Average p-value across monte carlos
for (num_sims in 1:10){
  print(mean(lrt.value.A2[,num_sims]))
}
```
Because the p-values are all greater than 0.05 we cannot reject the null model.
